...
export async function usuarioConsultaUsoIndevido(payload: any, authToken: string): Promise<any> {
  try {
    const body = encodeJWT(payload);
    const response = await axios.post(`${BILLING_URL}/vt-gateway/usuario/consulta-uso-indevido`, body, {
      headers: {
        "Content-Type": "application/json",
        Authorization: authToken,
      },
    ...
    throw new Error(error.response?.data?.message || "Falha ao consultar uso indevido de usuário");
  }
}
